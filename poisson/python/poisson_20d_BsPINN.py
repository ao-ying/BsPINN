...
# Generate dataset
def load_data(lb_geom, ub_geom, num_sample, num_boundary, dim_x, num_test):
    # train set
...
class BsPINN(nn.Module):
    def __init__(self, Layers, num_sample, num_boundary, boundary_weight, lb_X, ub_X, num_res, dim_x, dim_y, coe):
        ...
        self.dim_x = dim_x
        self.dim_y = dim_y
        self.dim_res_fc = Layers[0] # Width of fully connected networks in residual blocks, which must be the same for all layers
    ...
    def neural_net(self, X):
        # Data preprocessing, here lb and ub used for training and testing should be the same, otherwise the neural networks used for training and testing will be different
    ...
    # Predict the u value at points corresponding to X
    def predict(self, X):
    ...
    # Assign parameters to a new variable using .data, then .to(device)
    def set_device(self, device): 
    ...
    # Compute relative error: Approximately 5s per calculation
    def rel_error(self):
        # Data conversion
        self.set_device(device_cpu)
        
        # Calculate error
        u_pred = self.predict2(X_test).flatten()
        u_truth = Y_test 
...
if __name__ == "__main__":
    ...
    for seed in seeds: 
        ...
        dim_x = 20 # Dimension of x
        dim_y = 1 # Dimension of y
        coe = 0.4 * np.pi # Coefficient in sin
        
        ## Neural network related
        train = True
        name = "highBsPINN_256-32_Adam_v15"
        epochs = 30000 # Number of optimizer iterations
        num_sample = 10000
        num_boundary = 3000 # Must be divisible by dim_x
        num_test = 2000000
        Layers = [256, 128, 64, 32] # Fully connected network structure in each residual block, the number of neurons in each layer must be the same
        learning_rate = 0.001 # Initial learning rate 
        boundary_weight = 1 # Setting it too large may cause gradient explosion
        num_res = 2 # Number of residual blocks
        h = int(1000) # Learning rate decay related
        r = 0.95 # Learning rate is multiplied by r every h iterations
        weight_decay = 0.001 # L2 regularization coefficient, to prevent sudden increase in loss value
        ...
        lb_diff = -0.05
        ub_diff = 0.05
        error_param = 5 # Draw (20 * error_param) error points, each drawing consumes about 6s. epochs needs to be divisible by (20 * error_param).
        record_error = True
        ...
        if train:
            print("Loading data")
            ## Training data
            X_train, X_test, Y_test = load_data(lb_geom, ub_geom, num_sample, num_boundary, dim_x, num_test)
            lb_X = X_train.min(0) # Get the maximum value of each dimension in the training set
            ub_X = X_train.max(0) # Get the minimum value of each dimension in the training set
            lb_X = torch.from_numpy(lb_X).float().to(device)
            ub_X = torch.from_numpy(ub_X).float().to(device)
            X_train = torch.from_numpy(X_train).float().to(device)
            X_test = torch.from_numpy(X_test).float().to(device_cpu)
            ...
            for it in range(epochs):
                ...
                # Save loss value and relative error, and save the model with the smallest training loss
                ...
                if record_error and (it + 1) % (epochs/20/error_param) == 0:
                    u_L2RE = model.rel_error() # Each calculation takes about 6s
        ...
        # Calculate the relative error of u on the test set
