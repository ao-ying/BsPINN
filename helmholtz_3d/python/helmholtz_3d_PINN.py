...
class PINN(nn.Module):
    ...
    def neural_net(self, X):
        ...
        if self.mode == "test": # If you want to use the CPU, you must include the following code
            self.weights = [self.weights[i].to(device) for i in range(len(self.weights))]
            self.biases = [self.biases[i].to(device) for i in range(len(self.biases))]
