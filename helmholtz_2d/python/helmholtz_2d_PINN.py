...
def load_data(num_domain, num_boundary, nx_test, lb_geom, ub_geom):
    ...
    # boundary condition
    points = np.linspace([lb_geom,lb_geom],[ub_geom,lb_geom],num_boundary,endpoint=False) # down
    X_train = np.r_[X_train, points]
    points = np.linspace([ub_geom,lb_geom],[ub_geom,ub_geom],num_boundary,endpoint=False) # right
    X_train = np.r_[X_train, points]
    points = np.linspace([lb_geom,lb_geom],[lb_geom,ub_geom],num_boundary,endpoint=False) # left
    X_train = np.r_[X_train, points]
    points = np.linspace([lb_geom,ub_geom],[ub_geom,ub_geom],num_boundary,endpoint=False) # up
    X_train = np.r_[X_train, points]
...
class PINN(nn.Module):
    ...
    def neural_net(self, X):
        ...
        for l in range(0,num_Layers-2):
            W = self.weights[l]
            b = self.biases[l]
            H = torch.sin(torch.add(torch.matmul(H, W), b))
        W = self.weights[-1]
        b = self.biases[-1]
        Y = torch.add(torch.matmul(H, W), b)
        return Y
    
    # fully connected neural network of a channel
    def neural_net_channel(self, X, channel):
        ...
        W = self.weights[-1]
        b = self.biases[-1]
        Y = torch.add(torch.matmul(H[: , 16 * channel : 16 * (channel + 1)], W[16 * channel : 16 * (channel + 1) , :]), b)
        return Y

    # fully connected neural network of a layer
...
if __name__ == "__main__":
    ...
    for seed in seeds:
        ...
        Layers = [2, 256, 256, 256, 256, 256, 1] # fully connected neural network structure
        learning_rate = 0.001 
        boundary_weight = 100
        ...
        np.savetxt(output_path + "highhe_truth.txt", u_truth, fmt="%s", delimiter=' ')
        np.savetxt(output_path + "highhe_pinn.txt", u_pred, fmt="%s", delimiter=' ')
        ...
        #     np.savetxt(output_path + "per_channel_pinn/%d.txt" % channel, u_pred, fmt="%s", delimiter=' ')
        ...
        #     u_pred = model2.predict_layer(X_pred, num_layer).flatten()
        #     np.savetxt(output_path + "per_layer/%d.txt" % num_layer, u_pred, fmt="%s", delimiter=' ')