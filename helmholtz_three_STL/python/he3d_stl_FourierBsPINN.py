...
class FourierBsPINN(nn.Module):
    def __init__(self, Layers, kappa, num_domain, boundary_weight, lb_X, ub_X, map_size, scale):
    ...
    def neural_net(self, X):
        ...
        # Fourier feature mapping maps the input dimension to a 2 * map_size dimensional vector.
...
if __name__ == "__main__":
    ...
    for seed in seeds:
        ...
        train = True
        name = "FourierBsPINN_moredata"
        epochs = 20000 # Number of iterations for adam optimizer
        num_domain = 60000 
        num_boundary = 5000 
        num_domain_test = 10000 # 10000
        num_boundary_test = 10000 # 10000
        scale = 0.9
        map_size = 352
        Layers = [3, 352, 176, 88, 44, 22, 1] # Binary network structure
        ...
        print("\n\n***** name = %s *****" % name)
        print("seed = %d" % seed)
        output_path = path + ('./output_Bs_FourierPINN/')
        if not os.path.exists(output_path): os.mkdir(output_path)
        output_path = path + ('./output_Bs_FourierPINN/%s/' % name)
        if not os.path.exists(output_path): os.mkdir(output_path)
        output_path = path + ('./output_Bs_FourierPINN/%s/train_%d/' % (name, seed))
        if not os.path.exists(output_path): os.mkdir(output_path)
        ...
        if train:
            ...
            model = FourierBsPINN(Layers, kappa, num_domain, boundary_weight, lb_X, ub_X, map_size, scale)
            ...
            print("min_loss = %.4e" % min_loss)
        else:
            X_train_np, Y_train_np = load_data_file(X_train_interior_path, X_train_boundary_path, num_domain, num_boundary) # This step is to ensure that the test set is consistent with the training.
            print("Generating test set!") 
            X_test_np, Y_test_np = load_data_file(X_train_interior_path, X_train_boundary_path, num_domain_test, num_boundary_test)
            X_test = torch.from_numpy(X_test_np).float().to(device)
            Y_test = torch.from_numpy(Y_test_np).float().to(device)
