...
# FourierBsPINN Neural Network
class FourierBsPINN(nn.Module):
    def __init__(self, Layers, Nf, lb_X, ub_X, map_size, scale):
        super(FourierBsPINN, self).__init__()
        ...
        self.Layers[0] = 2 * self.map_size
        self.Nf = Nf
        self.act = torch.tanh
        self.num_Layers = len(self.Layers)
        self.params = []
        self.width = [self.Layers[0]] + [int(pow(2, i - 1) * self.Layers[i]) for i in range(1,len(self.Layers) - 1)] + [self.Layers[-1]] 
        self.masks = self.construct_mask()
        self.num_param = self.cal_param()
        # Initialize parameter matrix B
        self.B = np.random.normal(0, scale, (self.map_size, Layers[0]))
    ...
    def neural_net(self, X):
        ...
        # Data preprocessing, here the lb and ub used during training and testing should be the same, otherwise the neural network used for training and testing will be different.
        X = 2.0 * (X - self.lb_X) / (self.ub_X - self.lb_X) - 1.0
        X = X.float()
        # Fourier feature mapping, mapping the input dimension to a 2*map_size dimensional vector.
        x_proj = 2 * np.pi * torch.matmul(X, self.B.T)
        X = torch.concat([torch.sin(x_proj), torch.cos(x_proj)], dim=-1)
        # Fully connected network part
...
if __name__ == "__main__":
    ...
    for seed in seeds: 
        ...
        lb_diff = -0.00027
        ub_diff = 0.00027
        error_param = 5 # Draw (20 * error_param) error points.
        
        # Auxiliary variables
        name = name + ("_%d" % epochs)
        print("\\n\\n***** name = %s *****" % name)
        print("seed = %d" % seed)
        output_path = path + ('./output_Bs_fourier/')
        if not os.path.exists(output_path): os.mkdir(output_path)
        output_path = path + ('./output_Bs_fourier/%s/' % name)
        if not os.path.exists(output_path): os.mkdir(output_path)
        output_path = path + ('./output_Bs_fourier/%s/train_%d/' % (name, seed))
        if not os.path.exists(output_path): os.mkdir(output_path)
        ...
        if train:
            ...
            model = FourierBsPINN(Layers, Nf, lb_X, ub_X, map_size, scale)
        ...
        plt.xlabel('$x$')
        plt.ylabel('$t$')
        plt.title('$u$(FourierBsPINN)')
        if align:
            plt.savefig(output_path + "burgers_u_fourier_bspinn.png", format="png", dpi=150, bbox_inches="tight")
        else:
        ...
        plt.xlabel('$x$')
        plt.ylabel('$t$')
        plt.title('Point-wise Error(FourierBsPINN)', fontsize=20)
        if align:
            plt.savefig(output_path + "burgers_diff_fourier_bspinn3.png", format="png", dpi=150, bbox_inches="tight")
        else:
